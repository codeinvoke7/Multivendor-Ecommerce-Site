...
$num = 1;
$sql = $conn->query("SELECT * FROM `products` WHERE vendor_id = $id");
...
if ($sql->num_rows <= 0) {
	...
	<td>
		...
		if($row['status'] === '0'){
		?>
		<a href="/vendor/status/product/<?= $row['id'] ?>" title="active" class="btn btn-info text-white"><i class="bx bx-like  "></i></a>
        <?php
		}else{
			?>
					<a href="/vendor/status/product/<?= $row['id'] ?>" title="inactive" class="btn btn-info text-white"><i class="bx bx-dislike  "></i></a>
			<?php
		}
		?>
		<a href="/product/view<?= $row['id'] ?>" title="inactive" class="btn btn-warning text-white"><i class="bx bx-show"></i></a>
		<a href="/vendor/edit/product/<?= $row['id'] ?>" class="btn btn-info"><i class="bx bx-edit"></i></a>
        <a href="/vendor/delete/product/<?= $row['id'] ?>" class="btn btn-danger" id="delete"><i class="bx bx-trash"></i></a>
    </td> 
</tr>
<?php
$num++;
