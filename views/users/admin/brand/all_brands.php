...
<th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Brand Name : activate to sort column ascending" style="width: 287px;">Brand Name </th>
<th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Brand Image : activate to sort column ascending" style="width: 222px;">Brand Image </th>
<th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Action: activate to sort column ascending" style="width: 231px;">Status</th>
<th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Action: activate to sort column ascending" style="width: 231px;">Action</th>
</tr>
</thead>
<tbody>
...
if ($sql->num_rows <= 0) {
    ...
    <td> <img src="/views/public/uploads/brand/<?= $row['brand_image'] ?>" style="width: 70px; height:40px;">  </td>
	<td><?= $row['status'] === '1' ? '<div class="text-center"><p class="badge bg-primary">'. 'Active'. '</p></div>' :
	 '<div class="text-center"><p class="badge bg-danger">'. 'Inactive'. '</p></div>' ?></td>
	
	<td>
		<?php 
		if($row['status'] === '0'){
		?>
		<a href="/status/brand/<?= $row['id'] ?>" title="active" class="btn btn-info text-white"><i class="bx bx-like  "></i></a>
        <?php
		}else{
			?>
					<a href="/status/brand/<?= $row['id'] ?>" title="inactive" class="btn btn-warning text-white"><i class="bx bx-dislike  "></i></a>
			<?php
		}
		?>
		<a href="/edit/brand/<?= $row['id'] ?>" class="btn btn-info">Edit</a>
        <a href="/delete/brand/<?= $row['id'] ?>" 
