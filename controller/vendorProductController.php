...
if (isset($_POST['save'])) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: /vendor/all/products");
    exit();
...
if ($result) {
    ...
    $_SESSION['alert-type'] = 'success';
    header("Location: /vendor/edit/product/$product_id");
    exit();
}
...
function updateThumbnail($conn, $product_id)
{
    ...
    if ($_FILES['product_thumbnail']['error'] === UPLOAD_ERR_OK) {
       ...
       if ($result) {
        ...
        $_SESSION['message'] = 'Product thumbnail update failed. File is empty';
        $_SESSION['alert-type'] = 'error';
        header("Location: /vendor/edit/product/$product_id");
        exit();
    }
    ...
    if (isset($_POST['update_images'])) {
        foreach ($_FILES['multi_img']['tmp_name'] as $image_id => $tmp_name) {
            if ($_FILES['multi_img']['error'][$image_id] === UPLOAD_ERR_OK) {
                ...
                $_SESSION['message'] = 'Product multiImage update failed. File is empty';
                $_SESSION['alert-type'] = 'error';
                header("Location: /vendor/edit/product/$product_id");
                exit();
            }
...
if (isset($_SERVER['REQUEST_URI'])) {
   ...
   $categoryID = $categoryID['4'];
    if ($uri === '/vendor/subcategory/ajax/'. $categoryID) {
...
if (isset($_SERVER['REQUEST_URI'])) {
    ...
    if ($uri === '/vendor/update/product/' . $product_id) {
        updateProduct($conn, $product_id);
    }  elseif ($uri === '/vendor/status/product/' . $product_id) {
        toggleProductStatus($conn, $product_id);
    } elseif ($uri === '/vendor/delete/product/' . $product_id) {
...
if (isset($_SERVER['REQUEST_URI'])) {
    ...
    if ($uri === '/vendor/update/product/thumbnail/' . $product_id) {
        updateThumbnail($conn, $product_id);
    } elseif ($uri === '/vendor/update/product/images/' . $product_id) {
